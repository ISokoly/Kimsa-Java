...
export class ApiService {
  ...
  login(username: string, password: string): Observable<UsuarioLigero> {
    return this.http
      ...
      .pipe(
        ...
        map((res) =>
          res?.user ? (res.user as UsuarioLigero) : (res as UsuarioLigero)
        ),
        catchError((err) => {
          if (err?.status === 0) {
            return throwError(
  ...
  enableCategoriaYProductos(id: number): Observable<any> {
    return this.http.put(
      `${this.apiUrl}/categories/${id}/enable`,
      {},
      this.withCred({ headers: this.authHeaders() })
    );
  }
  ...
  createPayment(data: any): Observable<any> {
    ...
    console.log(
      "[API] POST /payments Authorization =>",
      headers.get("Authorization")
    );
    return this.http.post(
      `${this.apiUrl}/payments`,
      data,
      this.withCred({ headers })
    );
  }
  ...
  toggleSupply(id: number): Observable<any> {
    return this.http.patch(
      `${this.apiUrl}/supplies/${id}/toggle`,
      {},
      this.withCred({ headers: this.authHeaders() })
    );
  }
  ...
  updatePurchase(
    ...
    data: {
      idSupplier: number;
      items: Array<{ idSupply: number; quantity: number }>;
    }
  ): Observable<any> {
    return this.http.put<any>(
