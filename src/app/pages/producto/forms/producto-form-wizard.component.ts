...
export class ProductoFormWizardComponent implements OnInit, OnChanges {
  ...
  async ngOnInit(): Promise<void> {
    ...
    } catch { this.featuresBase = []; this.caracteristicasFiltradas = []; }
  ...
  private async cargarMarcasPorCategoria(idCategory: number): Promise<void> {
    try {
      ...
      this.marcas = (all || []).filter(m => Number((m as any).idCategory ?? m.idCategory ?? (m as any).category) === idCategory);
    } catch { this.marcas = []; }
  }
  ...
  /** Se dispara cuando el overlay de marcas se cierra (botón cerrar, guardar, etc.) */
  async alCerrarOverlayMarcas(): Promise<void> {
    if (this.categoriaId) await this.cargarMarcasPorCategoria(this.categoriaId);
    if (this.formData.brand && !this.marcas.some(m => m.idBrand === this.formData.brand)) {
  ...
  /** Si eliminan una marca desde el overlay */
  ...
  irPaso2(): void { if (this.requerirCamposBasicos()) this.formStep = 2; }
  volverPaso1(): void { this.formStep = 1; }
  ...
  reiniciarSeleccionImagen(): void { this.selectedFile = null; this.imagePreview = null; this.nombreArchivo = null; }

  /* ===== Helpers ===== */
  private minusculas(s: any) { return String(s ?? '').trim().toLowerCase(); }
  ...
  alEscribirCaracteristica(): void {
    // Si ya hay una característica seleccionada, NO permitir escribir ni filtrar
    if (this.newFeature.idFeature !== null) return;
    const q = this.minusculas(this.textoCaracteristica);
    if (!q) { this.caracteristicasFiltradas = [...this.featuresBase]; return; }
    this.caracteristicasFiltradas = this.featuresBase.filter(f => this.minusculas(f.featureName).includes(q));
  }

  alSeleccionarCaracteristica(idFeature: number | null): void {
    // Si ya hay una seleccionada, ignorar nuevas selecciones
    if (this.newFeature.idFeature !== null) return;
    ...
    const f = this.featuresBase.find(x => x.idFeature === idFeature);
    if (!f) return;
    this.newFeature.idFeature = f.idFeature;
    this.textoCaracteristica = f.featureName;
    this.caracteristicasFiltradas = [f]; // “fija” selección
  }
  ...
  eliminarCaracteristicaBorrador(f: { idFeature: number; featureName: string; featureValue: string }): void {
    this.draftFeatures = this.draftFeatures.filter(x => x.idFeature !== f.idFeature);
  }
  editarCaracteristicaBorrador(f: { idFeature: number; featureName: string; featureValue: string }): void {
  ...
  alEscribirInsumo(): void {
    // Si ya hay un insumo seleccionado, NO permitir escribir ni filtrar
    if (this.newRecetaItem.idSupply !== null) return;
    const q = this.minusculas(this.textoInsumo);
    if (!q) { this.insumosFiltrados = [...this.supplies]; return; }
    this.insumosFiltrados = this.supplies.filter(s => this.minusculas(s.name).includes(q));
  }

  alSeleccionarInsumo(idSupply: number | null): void {
    // Si ya hay un insumo seleccionado, ignorar nuevas selecciones
    if (this.newRecetaItem.idSupply !== null) return;
    ...
    this.insumosFiltrados = [s]; // “fija” selección
  }
  ...
  agregarItemReceta(): void {
    ...
    // limpiar selección de insumo y cantidad (para que puedas elegir OTRO insumo)
  ...
  /* ===== Imagen helper ===== */
  private async asegurarImagen(file: File | null, nombre: string, categoriaId: number, currentIdImage: number | null): Promise<number | null> {
  ...
  private minusculasCmp(s: string) { return (s || '').trim().toLowerCase(); }
      ...
      if (this.productoEditar) {
        ...
        if (newName !== currentName) {
          const conflict = (productos || []).some((p: any) =>
            this.minusculasCmp(p.name) === newName && Number(p.idProduct) !== idProduct
          );
          if (conflict) { this.isLoading = false; return this.toast.mostrarMensaje('❌ Ya existe un producto con este nombre.'); }
        }
        const idImage = await this.asegurarImagen(this.selectedFile, name, this.categoriaId, this.productoEditar.idImage ?? null);
        ...
        for (const pf of this.existingPF) existingByFeature.set(pf.idFeature, { idProductFeature: pf.idProductFeature, value: pf.featureValue });

        for (const d of this.draftFeatures) {
          const already = existingByFeature.get(d.idFeature);
          if (already) {
            if (this.minusculasCmp(already.value) !== this.minusculasCmp(d.featureValue)) {
              await firstValueFrom(this.api.updateProductFeature(already.idProductFeature, {
                featureValue: d.featureValue, product: { idProduct }, feature: { idFeature: d.idFeature }
              }));
            }
        ...
        if (this.existingRecipeId) await firstValueFrom(this.api.updateRecipe(this.existingRecipeId, { idProduct, items }));
        else await firstValueFrom(this.api.createRecipe({ idProduct, items }));
        ...
        // === CREATE ===
        const conflict = (productos || []).some((p: any) => this.minusculasCmp(p.name) === this.minusculasCmp(name));
        if (conflict) { this.isLoading = false; return this.toast.mostrarMensaje('❌ Ya existe un producto con este nombre.'); }

        const idImage = await this.asegurarImagen(this.selectedFile, name, this.categoriaId, null);
        const created: any = await firstValueFrom(this.api.createProducto({
          name, price: this.formData.price, category: { idCategory: this.categoriaId },
          brand: this.formData.brand ? { idBrand: this.formData.brand } : null,
        ...
        for (const pf of this.draftFeatures) {
          await firstValueFrom(this.api.createProductFeature({
            featureValue: pf.featureValue, product: { idProduct }, feature: { idFeature: pf.idFeature }
          }));
        }
        await firstValueFrom(this.api.createRecipe({
          idProduct, items: this.draftRecipe.map(d => ({ idSupply: d.idSupply, gramsQuantity: d.gramsQuantity }))
        }));
  ...
  /* ===== Precarga edición ===== */
  private async cargarDatosExistentesProducto(idProduct: number): Promise<void> {
    try {
      ...
      this.draftFeatures = this.existingPF.map(x => ({
        idFeature: x.idFeature, featureName: x.featureName, featureValue: x.featureValue
      }));
